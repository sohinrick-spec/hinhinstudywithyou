...
function StartScreen({
  ...
  dailyQuestStats, onClaimQuest, isAdminUnlocked
}) {
...
function TeacherStatsScreen({ onBack, leaderboardData, currentUserName, questionPool = [] }) {
  ...
  // 🆕 題庫對照表：用題目 ID 反查完整題目（錯題本本身不存題目文字）
      ...
      <div className="glass-panel p-4 md:p-6 rounded-2xl shadow-xl">
        <div className="flex items-center justify-between mb-6 border-b border-gray-200 dark:border-gray-700 pb-4 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <button onClick={onBack}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-slate-500 text-gray-600 dark:text-gray-300 text-sm font-medium transition-colors flex-shrink-0"
              title="返回主頁">
              <i className="fas fa-arrow-left"></i>
              <span className="hidden sm:inline">返回</span>
            </button>
        ...
        <CollapsibleSection
          ...
          {(() => {
            ...
            // 🆕 用多重索引反查題目：id / composite / 純標題
            const byId = {};
            const byComposite = {};
            const byTitle = {};
            for (const q of questionPool) {
              const idKey = getQuestionId(q);
              if (idKey) byId[idKey] = q;
              const t = String(q.title || q.question || '').trim().toLowerCase();
              if (t) {
                byTitle[t] = q;
                byComposite[`${q.category || ''}::${String(q.title || q.question || '').slice(0, 80)}`] = q;
              }
            }
            const resolveQ = (qId) => {
              if (byId[qId]) return byId[qId];
              if (byComposite[qId]) return byComposite[qId];
              if (typeof qId === 'string' && qId.includes('::')) {
                const titlePart = qId.split('::').slice(1).join('::').trim().toLowerCase();
                if (byTitle[titlePart]) return byTitle[titlePart];
                for (const q of questionPool) {
                  const t = String(q.title || q.question || '').trim().toLowerCase();
                  if (t && (t.startsWith(titlePart) || titlePart.startsWith(t.slice(0, 80)))) return q;
                }
              }
              return null;
            };
            ...
            Object.entries(wbAll).forEach(([studentName, wb]) => {
              ...
              Object.entries(wb).forEach(([qId, entry]) => {
                if (!entry) return;
                const wc = Number(entry.wrongCount) || 0;
                if (wc <= 0) return; // 沒真正答錯過的不列入

                const q = resolveQ(qId);
                if (!q) unresolved++;
              ...
              <div className="space-y-2">
                {unresolved > 0 && (
                  <div className="text-[11px] text-amber-600 dark:text-amber-400 mb-1">
                    ...
                    有 {unresolved} 筆錯題無法對應到題庫（題目可能已被編輯或移除），以下用題目編號顯示。
                  </div>
                )}
                {sorted.map((item, i) => {
