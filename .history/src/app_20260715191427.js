function BiologyFlashcardApp() {
  ...
  const [adminModal, setAdminModal] = useState({ open: false, target: null, pwd: '', loading: false, error: '' });
...
const [mySkipCards, setMySkipCards] = useState(0);
  ...
  // 🆕 老師身份自動視為管理員
  const isTeacherUser = useMemo(() => isTeacher(userName), [userName]);
  // 🆕 最終管理員權限 = 老師登入 或 密碼解鎖
  const isAdmin = isAdminUnlocked || isTeacherUser;
...
// 🆕 每日任務：累計今日作答題數
  ...
  const requireAdmin = useCallback((targetState) => {
    if (isAdmin) { setGameState(targetState); return; }
...
{gameState === 'simulation' && (
  <motion.div key="simulation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    className="fixed inset-0 z-[9999] flex flex-col bg-white dark:bg-gray-900">
   <div className="flex items-center gap-3 px-4 py-2 bg-emerald-600 text-white shadow-md flex-shrink-0">
  <button onClick={() => setGameState('start')}
    className="flex items-center gap-1.5 text-sm font-bold hover:bg-white/20 px-3 py-1.5 rounded-lg transition-all">
    <i className="fas fa-arrow-left"></i> 返回主頁
  </button>
  <button onClick={() => {
    const iframe = document.querySelector('iframe[title="互動模擬"]');
    if (iframe) iframe.src = 'https://sohinrick-spec.github.io/hinhinsimulation/';
  }}
    className="flex items-center gap-1.5 text-sm font-bold hover:bg-white/20 px-3 py-1.5 rounded-lg transition-all">
    <i className="fas fa-home"></i> 模擬器主頁
  </button>
  <span className="font-bold text-base">🎮 互動模擬</span>
</div>
    <iframe
      src="https://sohinrick-spec.github.io/hinhinsimulation/"
      className="flex-1 w-full border-0"
      allow="autoplay; fullscreen"
      title="互動模擬"
    />
