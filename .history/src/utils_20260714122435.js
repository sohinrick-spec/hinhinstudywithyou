...
function isGraduated(fullName, lastActiveDateStr) {
  ...
  // 計算「當前學年」的畢業日 = 該學年的 5/31
  // 學年 9 月開始:若現在是 1–8 月,畢業日屬今年;若 9–12 月,屬明年
  const y = hkt.getFullYear();
  const gradYear = hkt.getMonth() >= 8 ? y + 1 : y;   // 9月(idx8)起算下一年
  const gradDeadline = new Date(gradYear, 4, 31);      // 該學年 5/31

  // 未過畢業日 → 一定是在學,不隱藏(這樣新一屆中六 9–12 月正常顯示)
  if (hkt < gradDeadline) return false;

  // 已過畢業日:只隱藏「畢業後就沒再活動」的人
  // 若該生在畢業日之後仍有活動,代表其實是新一屆,不該隱藏
